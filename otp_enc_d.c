...
int main(int argc, char* argv[]){
	...
	while(1){
		...
		if(fork() == 0){
			...
			//In child, verify that incoming request is from otp_enc_d, which
			//sends a preliminary code of "enc" as an identifier. If the request
			//does not contain this code (i.e. is not from otp_enc, send an 
			...
			if(strcmp(validate, "enc") != 0){
				if((charsSent = send(establishedConnectionFD, "enc NO ACK", 10, 0)) < 0){
					error("ERROR: Unable to send enc NO ACK ");
				}
				write(STDERR_FILENO, "ERROR: Must send encryption request from otp_enc\n", 49);
				exit(1);
			}
			if((charsSent = send(establishedConnectionFD, "enc ACK", 7, 0)) != 7){
				error("ERROR: Unable to send enc ACK ");
			}

			//Strings to hold buffers for plaintext, key text, and the total character count
			//for incoming data.
			char ptBuffer[513], kBuffer[513], tccs[33];

			//otp_enc next sends the total character count for the pending data.
			...
			//Recv plaintext data followed by key text data. Encrypt and send back
			//to otp_enc. If bad data is detected, print error message and exit.
			for(int i = 0; i < numRecvs; i++){
				
				//Receive up to 512 characters of plaintext, store in ptBuffer
				memset(ptBuffer, '\0', 513);
				if((charsRead = recv(establishedConnectionFD, ptBuffer, 512, 0)) < 0){
					error("ERROR: Unable to read plaintext data ");
				}
				...
				//Check for invalid plaintext character using regular expression compiled above.
				//Print error and exit if an invalid character is detected.
				if(regexec(regex, ptBuffer, 0, NULL, 0) == 0){
					fprintf(stderr, "ERROR: Invalid character(s) in plaintext file\n");
					if((charsSent = send(establishedConnectionFD, "bad ch", 6, 0)) < 0){
					error("ERROR: Unable to send bad ch for plaintext segment receipt ");
				}
					exit(1);
				}
				
				//Acknowledge receipt of encrypted data segment
				if((charsSent = send(establishedConnectionFD, "pt recvd", 8, 0)) < 0){
					error("ERROR: Unable to send ack for plaintext segment receipt ");
				}
				 ...
				 ******************************************************
				//Check for invalid character(s) in plain text or key from client,
				//and exit if present.
				for(int j = 0; j < strlen(ptBuffer); j++){
					if((ptBuffer[j] < 65 || ptBuffer[j] > 90) && ptBuffer[j] != 32){
						write(STDERR_FILENO, "ERROR: Bad plaintext data\n", 26); 
						exit(1);
					}
				}
				...
				**********************************************************/
				...
				if((charsRead = recv(establishedConnectionFD, validate, 3, 0)) != 3){
					write(STDERR_FILENO, "ERROR: No receipt message from client\n", 38); 
					exit(1);
				}

				//Encrypt the buffer. (Note the renaming of ptBuffer to encBuffer during this
				//process isn't strictly necessary, but is used to provide a more accurate
				//nomenclature, as by the time of sending, the buffer is no longer "plaintext",
				//since it has been encrypted.)
				char *encBuffer = ptBuffer;
				for(int j = 0; j < strlen(ptBuffer); j++){
					
					//If the plaintext or key character is a space, convert it to the
					//value that immediately follows "Z" in the ASCII table, so the 
					//encryption formula below will still be valid for it.
					if(ptBuffer[j] == 32){encBuffer[j] = 91;}
					if(kBuffer[j] == 32){kBuffer[j] = 91;}

					if((encBuffer[j] = (encBuffer[j] - 65) + (kBuffer[j] - 65)) > 26){
						encBuffer[j] -= 27;
					}
				...
				//Send back to otp_enc
				if((charsSent = send(establishedConnectionFD, encBuffer, strlen(encBuffer), 0)) < 0){
					error("ERROR: Unable to send encrypted block ");
				}
			}
