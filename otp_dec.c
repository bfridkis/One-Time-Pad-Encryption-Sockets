...
** Author: Ben Fridkis
** Date: 11/21/2018
** Description: Client program for otp_dec_d. Initiates requests to otp_dec_d to
...
void cError(int port) { fprintf(stderr, "OTC_DEC: Connection error on port: %d\n", port); exit(2); }
...
int main(int argc, char* argv[]){
	...
	char etBuffer[513], kBuffer[513];

	//Check usage and args
	if (argc < 4) { 
		fprintf(stderr,"USAGE: %s port [encrypted filename] "
						"[key filename] port\n", argv[0]);
		exit(1); 
	}

	//Compare key file size to encrypted file size. If key is smaller
	//print error and exit.
	if(fsize(argv[1]) > fsize(argv[2])){
		fprintf(stderr, "OTP_DEC: Key file size < encrypted file size\n");
		exit(1);
	}

	//Open both files (encrypted and key).
	//Print error message and exit if either cannot be opened.
	int et;
	if((et = open(argv[1], O_RDONLY)) == -1){
		error("OTP_DEC: Could not open encrypted file ");
	}
	int k;
	if((k = open(argv[2], O_RDONLY)) == -1){
		error("OTP_DEC: Could not open key file ");
	}

	//Check files for invalid characters
	//(See validateFiles.c for more details)
	validateFiles(et, k, 0);
	...
	if((charsWritten = send(socketFD, "dec", 3, 0)) < 0){
		error("OTP_DEC: Error writing to socket ");
	}
	
	//Receive acknowledgement from server, error if "dec NO ACK" is sent
	char ackResponse[11];
	memset(ackResponse, '\0', 11);
	if((charsRead = recv(socketFD, ackResponse, 10, 0)) < 0){
		error("OTP_DEC: Unable to read ACK response from server ");
	}
	if(strcmp(ackResponse, "dec ACK") != 0){
		fprintf(stderr, "OTP_DEC: Unable to connect to server - bad server/client combo\n");
		exit(2);
	}
	
	//Convert the encrypted file size to a string and send to server.
	...
	if((charsWritten = send(socketFD, fileSizeString, strlen(fileSizeString), 0)) < 0){
		error("OTP_DEC: Unable to send file size ");
	}

	//Receive server's acknowledgment that the file size message has been
	//received.
	memset(ackResponse, '\0', 11);
	if((charsRead = recv(socketFD, ackResponse, 9, 0)) != 9){
		error("OTP_DEC: Server unable to acknowledge encrypted character count ");
	}

	//Rewind file pointer offsets (for the encrypted and key files) in preparation
	//for data send operations.
	if(lseek(et, 0, SEEK_SET) == -1){
		error("OTP_DEC: Unable to rewind encrypted file for send operations ");
	}
	if(lseek(k, 0, SEEK_SET) == -1){
		error("OTP_DEC: Unable to rewind key file for send operations ");
	}
	
	//This final while loop handles the main data processing, sending segements
	//of 512 bytes for first the encrypted file and then the key file, then 
	//receiving and printing to stdout the decrypted result from the sever.
	int segmentSize;
	char response[513];
	while(memset(etBuffer, '\0', sizeof(etBuffer)) &&
			(segmentSize = read(et, etBuffer, sizeof(etBuffer) - 1)) != 0){
		if(segmentSize == -1){
			error("OTP_DEC: Unable to read encrypted file for send operation ");
		}
		//Do not send trailing newline if present (only applies for final segment).
		if(etBuffer[strlen(etBuffer) - 1] == '\n'){
			segmentSize--;
		}

		//Send encrypted, up to sizeof(etBuffer) - 1 bytes
		if((charsWritten = send(socketFD, etBuffer, segmentSize, 0)) < 0){
			error("OTP_DEC: Unable to send encrypted data segment ");
		}
		
		//Receive ack from server that encrypted bytes have been received
		memset(ackResponse, '\0', 11);
		if((charsRead = recv(socketFD, ackResponse, 8, 0)) < 0){
			error("OTP_DEC: No ACK from server after sending encrypted data segment ");
		}
		if(strcmp(ackResponse, "bad ch") == 0){
			fprintf(stderr, "OTP_DEC: Bad character sent in encrypted segment\n");
			exit(1);
		}
		
		if((read(k, kBuffer, segmentSize)) != segmentSize){ 
			error("OTP_DEC: Unable to read key file for segment corresponding "
				  "to encrypted data ");
		}
		//Send key data, up to size of (etBuffer) - 1 bytes, corresponding to encrypted 
		...
		memset(ackResponse, '\0', 11);
		if((charsRead = recv(socketFD, ackResponse, 8, 0)) < 8){
			error("OTP_DEC: No ACK from server after sending key data segment ");
		}
		if(strcmp(ackResponse, "bad ch") == 0){
			fprintf(stderr, "OTP_DEC: Bad character sent in key segment\n");
			exit(1);
		}
		
		//Send ready message to server, then receive result segment
		if((charsWritten = send(socketFD, "rdy", 3, 0)) < 0){
			error("OTP_DEC: Unable to send receipt ready message to server ");
		}
		memset(response, '\0', sizeof(response));
		if((charsRead = recv(socketFD, response, sizeof(response) - 1, 0)) < 0){
			error("OTP_DEC: Unable to receive decrypted data segment from server ");
		}
